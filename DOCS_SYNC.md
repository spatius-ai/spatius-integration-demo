...
| LiveKit demo (avatar in the room) | `livekit-demo/clients/*`, `livekit-demo/servers/*` | `getting-started/how-to-integrate`, `livekit-agents/*`, `resources/demo-projects`, Web RTC Adapter references |
| Agora demo (avatar in the channel) | `agora-demo/clients/*`, `agora-demo/servers/*` | `getting-started/how-to-integrate`, `agora-convoai/*`, `resources/demo-projects`, Web RTC Adapter and mobile RTC SDK references |
...
- Keep the demos presented as one set. Each has a server that holds the credentials and a client that starts by picking a scene on a configuration page; they differ only in who connects to Motion Server — the client (Direct), the server (Backend), or neither, with the avatar itself in the call (the LiveKit and Agora demos, which differ in where the agent runs).
- Keep the two scenes named consistently across modes: **Sample audio** for the bundled clip, **Realtime conversation** for the microphone path. The LiveKit and Agora demos are realtime only.
