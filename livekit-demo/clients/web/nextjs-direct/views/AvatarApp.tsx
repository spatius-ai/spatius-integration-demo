...
/**
 * The app, in its own component so the page can defer the whole thing with
 * `next/dynamic` — see app/page.tsx for why the SDK cannot be imported into the
 * server pass.
 *
 * It boots straight into the room. There is no configuration step: every credential
 * and every setting — language, voice, model — lives in the server's `.env`, and the
 * server refuses to start without them. All this page needs is the App ID to
 * initialize the SDK with, which it fetches on mount; the only thing the user
 * chooses is a character.
 */
