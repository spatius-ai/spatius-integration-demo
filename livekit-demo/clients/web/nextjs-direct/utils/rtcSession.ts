 ...
 *   LiveKit   client ◄────  LiveKit room  ────► agent     (neither — it is in the room)
 *
 ...
 * `LiveKitProvider` is the one used here; the Agora demo swaps in `AgoraProvider` with
...
export class RtcSession {
  ...
  private provider: LiveKitProvider | null = null
   ...
   *
   * The agent is dispatched asynchronously after the session call returns, and audio
   * sent before it is up is simply dropped — the symptom is a room that connects but
   * never answers.
   *
   * "The agent joined" is not enough: at join time AgentSession is still starting up
   * and audio arriving then is dropped. The worker sets a `ready` attribute after
